"""B56 / Worker 4 — shared harness for the live validation arms.

Everything the individual arms share: the control surface, the on-disk oracles,
the gate, the S65-guarded log reader and the teardown. Split out of the arm
scripts because B56 runs FIVE arms across THREE restarts and a copy-pasted
harness is how two arms end up disagreeing about what `role` means.
...
----------------------------------------
  ...
  S85   every settle is `await_state` on an OBSERVABLE. There is not one
        `time.sleep` used as a wait-for-the-system-to-do-something in this file.
  rule 15 every signature carries its historical-hit control.
  secrets  no `data.json` VALUE is ever read except `sharedFolder`,
        `e2eControlPort` and `debugLogPath` — the three the rig's own guards
        need. Everything else is compared as sha256-of-bytes or not at all.
"""
...
def flush_log(name: str) -> None:
    Path(rf"H:\tmp\b56_{name}_{RUN}.log").write_text("\n".join(OUT), encoding="utf-8")


def dump(name: str, payload: dict) -> None:
    Path(rf"H:\tmp\b56_{name}_{RUN}.json").write_text(
        json.dumps(payload, indent=2, default=str), encoding="utf-8")
    flush_log(name)
    say(f"  machine-readable: H:\\tmp\\b56_{name}_{RUN}.json")
...
# --------------------------------------------------------------------------- #
# the seed-refusal store — read and (for the CONTROL arm only) edited
# --------------------------------------------------------------------------- #
...
def gate(title: str, needed: tuple[str, ...] = ("session.info",),
    ...
    say("=" * 78)
    say(f"B56 / W4 — {title}   run {RUN}   {time.strftime('%Y-%m-%d %H:%M:%S')}")
    say("=" * 78)
    ok = True
    ...
    for role in VAULTS:
        for name in needed:
            if name == "session.info":
                ...
                # nothing to undo. What is being established is that the COMMAND
                # is routed on this build, which is the S57 question.
                r = cmd(role, name, op={"type": "delete",
                                        "path": f"{SHARED}/b56-route-probe-{RUN}.md"},
                        settleMs=200)
            else:
                r = cmd(role, name)
            blob = json.dumps(r)
...
def rule15(ev, extra: tuple[str, ...] = ()) -> dict:
    ...
    for sig in tuple(RECEIPTS) + tuple(extra):
        ...
        say(f"    {sig!r:26} window_hits={n:<5} verdict={v:<18} "
...
def restart(expect_digest: str, note: str = "") -> bool:
    """Restart both instances through the pinned installer. Blocks until both
    control servers answer `session.info` with a `vaultId` — a STATE wait, never
    a duration."""
    say("")
...
# --------------------------------------------------------------------------- #
# teardown — removes only records THIS run made (I11: nothing else is deleted)
# --------------------------------------------------------------------------- #
def drop_canvas(rel: str) -> None:
    """Remove a canvas this run created, from both vaults, and say what was left."""
    say(f"  ---- teardown {rel} ----")
    for role in VAULTS:
        p = vpath(role, rel)
        if p.exists():
