...
describe("WP44 AC4 — resolvePort precedence, observed through the control server", () => {
  it("binds the numeric LIVESHARE_E2E port (order 1)", async () => {
    ...
    // WP46 (BUILD_SPEC §7 amendment ledger, cross-licensed into this WP44 file) —
    // `session.info` now also reports vault, build and canvas-surface identity.
    // The subject here is still port precedence; the payload is only proof that a
    // real control server owns the port. Still an exact whole-object `toEqual`
    // over all nine keys: this fixture has neither `app`/`manifest` nor
    // `canvasSync`, so it additionally pins the AC3 honest-degradation values.
