# WP70 / AC1 — blind counterpart 1 for "port-only provisioning is byte-identical to the
# pre-change `_with_port` implementation".
#
# Different data: JSON structural characters living INSIDE string values, an escaped
# quote, an escaped backslash, deep nesting, and the port key name spelled in a value.
#
...
# Structural characters inside STRING values, and the port key spelled inside a value —
# a splice that scans for the key without honouring string boundaries corrupts these.
CORPUS = {
    "brace_in_string": b'{\n  "note": "a } and a { inside a string"\n}\n',
    "bracket_in_string": b'{\n  "note": "[not, an, array]"\n}\n',
    "escaped_quote_in_string": b'{\n  "note": "he said \\"hello\\" twice"\n}\n',
    "key_name_inside_a_value": b'{\n  "note": "e2eControlPort lives in constants.py"\n}\n',
    "colon_in_string": b'{\n  "url": "http://example.invalid:8080/path"\n}\n',
    "deep_nesting": b'{\n  "a": {"b": {"c": [{"d": [[[1]]]}]}}\n}\n',
    "empty_string_values": b'{\n  "a": "",\n  "b": ""\n}\n',
    "escaped_backslash_before_quote": b'{\n  "p": "C:\\\\path\\\\"\n}\n',
}
...
def test_structural_characters_in_values_do_not_change_the_port_only_bytes(
...
@pytest.mark.parametrize("label", sorted(CORPUS))
def test_each_corpus_entry_is_still_valid_json_after_the_splice(
    tmp_path: Path, label: str
) -> None:
    original = CORPUS[label]
    vault = make_vault(tmp_path, "json-" + label, original)
    ports.provision_port(vault, constants.ROLE_A)
    parsed = json.loads((vault / constants.PLUGIN_DATA_REL).read_bytes().decode("utf-8-sig"))
    assert parsed[constants.SETTINGS_PORT_KEY] == constants.REAL_CONTROL_PORT_A


def test_the_port_key_inside_a_string_value_is_not_spliced(tmp_path: Path) -> None:
    original = CORPUS["key_name_inside_a_value"]
    vault = make_vault(tmp_path, "vault-value-key", original)
    ports.provision_port(vault, constants.ROLE_A)
    parsed = json.loads((vault / constants.PLUGIN_DATA_REL).read_bytes().decode("utf-8"))
    assert parsed["note"] == "e2eControlPort lives in constants.py"


@pytest.mark.parametrize("label", sorted(CORPUS))
def test_the_full_cycle_returns_the_original_bytes(tmp_path: Path, label: str) -> None:
    original = CORPUS[label]
    vault = make_vault(tmp_path, "cycle-" + label, original)
    ports.provision_port(vault, constants.ROLE_A)
