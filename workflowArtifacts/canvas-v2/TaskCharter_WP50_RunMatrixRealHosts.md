...
<!-- Updated: AC5's second sync engine corrected from lan-vault-sync (installed, NOT enabled) to obsidian-git, and its disposition changed from a choice to a precondition; the driver's discarded `applied` split out as WP73; the location of the target file flagged; AC1–AC4 and AC6 unchanged verbatim 2026-08-04 -->
...
- **Entry points / relevant files (from `workflowArtifacts/RepoMap.md`, the V2 Touchpoint Inventory, and the e2e-infra survey):**
  ...
  - <!-- Updated: the target file is not in this repository; flagged for the Dispatcher rather than resolved here 2026-08-04 --> **⚠ Location note — do not re-derive, and do not "fix" the paths above by guessing.** `tools/MCPserver/liveshare_e2e_mcp_server.py` **does not exist in this repository**; this repo's `tools/` contains only `launch_liveshare_e2e.py`, `launch_obsidian_e2e.py` and `obsidian_e2e/`. The file lives in the AgenticWorkspace repo at `h:\My Code\AgenticWorkspace\tools\MCPserver\liveshare_e2e_mcp_server.py`. **The line references above are accurate against that file and are not the problem** — verified: `_wait_both` `:134`, `assert_converged` `:280` (decorator; `def` at `:281`), `run_matrix` `:402` (decorator; `def` at `:403`), `_run_case` `:321`, `MATRIX_CASES` `:45–52`. Only the *repository* is wrong. Two consequences the implementor must not absorb silently: the change lands **outside this project's branch** and therefore outside §7's commit and abort accounting, and this WP's standalone Python verification script cannot sit beside the file it verifies under the usual `tools/test_<name>.py` convention. **Unverified: whether the Dispatcher intends the edit in the workspace repo or the file vendored into this one. ESCALATE rather than choosing.**
...
- <!-- Updated: measured host realities entered from T3_PREFLIGHT.md 2026-08-02 --> **Known risks specific to this WP, measured on this host 2026-08-02 — given, do not re-derive:**
  ...
  - <!-- Updated: CORRECTED — the engine named here was lan-vault-sync, which is installed but NOT enabled; the enabled, never-dispositioned engine is obsidian-git, and AC5 now fixes its disposition rather than offering a choice 2026-08-04 --> **`obsidian-git` is enabled in both vaults with `autoPullOnBoot: true`**, over real git working trees with `origin` remotes already dirty (13 and 14 entries). It is a second sync engine and it fires **at the moment the gate launches Obsidian**: an auto-pull onto a dirty tree can merge, conflict or check out over local state before any Canvas V2 code runs, and the resulting failure would look like a sync bug without being one. **AC5 no longer offers a choice.** The disposition is a **precondition**: `obsidian-git` is disabled in both vaults for the duration of the run and restored afterwards, and both the disabling and the restoration are recorded with the run. `autoSaveInterval` / `autoPushInterval` are `0`, so nothing is pushed; the boot-time pull alone is disqualifying. **`lan-vault-sync` is installed but NOT enabled and cannot run** — recorded in that form only so a later reader does not rediscover it as a hazard, and it needs no disposition. Both plugins must survive the run — neither is uninstalled, and `community-plugins.json` is restored to its prior bytes.
  - <!-- Updated: standing lesson from the corrected claim above 2026-08-04 --> **Standing lesson — "installed" is not "enabled".** The false claim this bullet replaces was produced by reading the plugins **directory listing** and reporting it as the enabled set. The enabled set is `community-plugins.json`, per vault, and nothing else. Read it and record what it contained.
  - <!-- Updated: the rig has no launch backend, measured 2026-08-04 --> **The instances this driver talks to are started by an agent, not by the rig.** `lifecycle.py`'s only console backend is `PlanOnlyConsole` and there is no process spawn anywhere under `tools/obsidian_e2e/`; the gate run is agent-mediated (WP71 / C71). That is consistent with this WP's own shape — it is an **MCP server**, so its tools were always going to be called by an agent — and it changes nothing about these criteria. What it does change is what the driver may **assume**: it may not assume the endpoint it holds was brought up by the same process that is calling it, and every precondition AC5 lists must be established by the driver itself rather than inherited from a launcher.
  - <!-- Updated: the driver's own vacuity, now chartered as WP73 2026-08-04 --> **⚠ This driver currently cannot honour AC3.** `_run_case` discards the `applied` flag `canvas.simulateEdit` returns, at **all eleven** `_simulate` call sites (six is the case count, not the call-site count). An unapplied gesture therefore leaves both snapshots equal, `_converge_and_check` reports `converged: True`, and the case records **pass** — which AC3 forbids in as many words. The repair is chartered as **WP73 / C73** and is not folded into this WP; AC1–AC6 here are unchanged. Do not implement AC3 in a way that presumes the flag is already consumed.
