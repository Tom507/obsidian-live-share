# Task Charter — WP32: Receive-and-Persist mode
...
- **Outcome:** a degraded client is a reader, never a second writer.
- **BUILD_SPEC reference:** `workflowArtifacts/canvas-v2/BUILD_SPEC_CanvasV2.md` — section 5, component **C32 — Receive-and-Persist degradation mode** (work package WP32); phase **P3**.
...
- **In scope:**
  ...
  - Scope summary: read-only degradation with retry; unifies version mismatch
- **Out of scope / non-goals:**
  - Removing the old fallback door — WP33 (this WP builds the replacement first).
...
- **Known interfaces / dependencies:**
  ...
  - Depends on work packages: WP31
  - **Convergence-fuzzer link (required by the BUILD_SPEC):** WP23 "degraded replica" op — a replica in Receive-and-Persist emits **zero** updates for the run while still converging to the same state as every other replica.
...
- **Component(s) being changed:** Receive-and-Persist degradation mode
- **Interfaces involved:**
  - Input: a failure to reach full canvas mode (waitForSync timeout, doc error, schema-major mismatch)
  - Output: a read-only-but-persisting subscription with periodic retry
...
- **Entry points / relevant files (from `workflowArtifacts/RepoMap.md`, V2 Touchpoint Inventory):**
  - `plugin/src/files/vault-events.ts:101–117` — `subscribeCanvasWithHandover`
...
*Exact copy from BUILD_SPEC section 5, component C32. No paraphrasing.*
...
- **Required report:** `ImplementationReport_WP32.md` (in `workflowArtifacts/canvas-v2/`)
