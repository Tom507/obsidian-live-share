...
| **S95** | **An externally created file is never noticed by a running instance** — 0 receipts in 90 s — while an external *modify* lands in 2.0 s. | **open.** Create and modify are not symmetric on the inbound side, and only modify has ever been measured. Every prior *"external writes are seen"* claim rests on the modify arm. |
