...
| **S78** | **A NODE DELETION BY WHOLE-FILE WRITE IS NEVER CAPTURED.** Delta-independent out to 6 s on a 45 s budget, and **identical on both bundles** — so it is **pre-existing, not a WP91 regression**. Signature: `local modify` = 1, `declines` = 0. **The event arrives, capture runs, nothing declines — and the removal does not reach the doc.** | **open, and on the evidence the most severe unowned defect in the project.** WP91 fixed the case where a write is *never seen*; this is a write that **is** seen, is **not** declined, and still loses the user's intent. A user deleting a card sees it vanish locally and stay on every peer. **The green that covered this was already known vacuous.** Found live by B50. |
