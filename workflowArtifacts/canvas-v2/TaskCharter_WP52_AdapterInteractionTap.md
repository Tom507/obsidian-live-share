# Task Charter — WP52: Adapter interaction tap

**Charter Status:** `SPEC_COMPLETE`
**WP:** WP52
**Phase:** P5 (PHASE T3 group)
**task_mode:** `standard`
**Depends on:** WP39, WP46
**W4 Test Targets:** `0`
...
- **Outcome:** what Obsidian actually fires is recorded rather than assumed.
- **BUILD_SPEC reference:** `workflowArtifacts/canvas-v2/BUILD_SPEC_CanvasV2.md` — section 5, **PHASE T3**, component **C52 — Adapter interaction tap over the control protocol** (work package WP52); phase **P5**.
...
- **In scope:**
  ...
  - Scope summary: ordered, path-scoped signal events over the control protocol
- **Out of scope / non-goals:**
  - Changing which signals the adapter patches; the tap observes the existing patch set, it does not extend it (extending it is a possible WP54 outcome).
...
- **Known interfaces / dependencies:**
  ...
  - Depends on work packages: WP39, WP46
...
- **Component(s) being changed:** Adapter interaction tap over the control protocol
- **Interfaces involved:**
  - Input: the adapter's patched signals and the binding's capture counters
  - Output: an ordered, path-scoped event stream over the existing SSE channel
...
- **Entry points / relevant files (from `workflowArtifacts/RepoMap.md`, the V2 Touchpoint Inventory, and the e2e-infra survey):**
  - `plugin/src/canvas/canvas-adapter.ts:240–245` — `PatchName = "updateSelection" | "setDragging" | "markViewportChanged"`
...
*Exact copy from BUILD_SPEC section 5, PHASE T3, component C52. No paraphrasing.*
...
- **Required report:** `ImplementationReport_WP52.md` (in `workflowArtifacts/canvas-v2/`)
