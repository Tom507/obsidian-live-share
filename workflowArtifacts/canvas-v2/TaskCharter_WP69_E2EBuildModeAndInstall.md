...
- **Known risks specific to this WP:**
  ...
  - ~~**`lan-vault-sync` is enabled in both vaults**~~ — **FALSE; corrected 2026-08-04 after WP69 closed.** `community-plugins.json` is Obsidian's *enabled* list, it is byte-identical in both vaults, and it contains exactly `obsidian-git` and `live-share`. `lan-vault-sync` is **installed but not enabled** and cannot run. The claim came from the Dispatcher reading the plugins *directory listing* as the enabled set; it propagated into four other charters, which were corrected on 2026-08-04, and this one was missed because WP69 had already closed. **Standing lesson: "installed" is not "enabled".** The paragraph's *reasoning* survives intact and now points at a real engine: **`obsidian-git` is enabled in both vaults with `autoPullOnBoot: true`**, over dirty git work trees with `origin` remotes, and it can revert or propagate an installed bundle at launch. Its disposition is no longer WP50's choice — it is a **precondition ruled by the Dispatcher and mechanised in WP70** (borrow, disable, restore, verify the restore). This does not reopen WP69: its install is verified by byte comparison against a restore point, which holds whichever engine is live.
