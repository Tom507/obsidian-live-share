...
- **Outcome:** typing is never interrupted, and nothing is lost by deferring.
- **BUILD_SPEC reference:** `workflowArtifacts/canvas-v2/BUILD_SPEC_CanvasV2.md` — section 5, component **C37 — `isBusy()` extended with inline editing, deferred apply and blur merge** (work package WP37); phase **P4**.
...
- **In scope:**
  ...
  - Scope summary: editing-aware busy predicate, per-record queue
- **Out of scope / non-goals:**
  - Changing the drag watchdog behaviour or `DRAG_WATCHDOG_MS`.
...
- **Component(s) being changed:** `isBusy()` extended with inline editing, deferred apply and blur merge
- **Interfaces involved:**
  - Input: an "inline editor focused" signal
  - Output: per-record deferral with merge on blur
...
- **Entry points / relevant files (from `workflowArtifacts/RepoMap.md`, V2 Touchpoint Inventory):**
  - `plugin/src/canvas/canvas-adapter.ts:87` (interface), `:572` (impl), doc `:310` — `isBusy`; `DRAG_WATCHDOG_MS = 5000` at `:260`; second consumer at `:589`
...
*Exact copy from BUILD_SPEC section 5, component C37. No paraphrasing.*
...
- **Required changed files:**
  ...
  - a deferral-queue module
- **Required report:** `ImplementationReport_WP37.md` (in `workflowArtifacts/canvas-v2/`)
