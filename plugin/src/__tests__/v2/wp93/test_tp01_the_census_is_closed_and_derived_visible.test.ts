...
describe("WP93 AC1 — the consumer census, derived", () => {
  it("T1 — every `isPathMuted` call in production is derived, and the three counts that differ are all reported", () => {
    ...
    expect(byFile(CENSUS.consumers)).toEqual([
      // 7 gate expressions (`onFileRename` asks TWICE on one line = 8 calls)
      // + 1: S120's `isPathMutedFor` asks the refcount before asking the kind.
      "files/file-ops.ts×9",
    ...
    expect(CENSUS.consumers).toHaveLength(9);
    expect(lines.size).toBe(8);

    // And DECISIONS is a third number, still: `file-ops.ts` asks twice on the
    // `onFileRename` line, so 9 calls fall on 8 lines and each line is one
    // decision. Post-S120 all three numbers live in one file.
    ...
    expect({ calls: CENSUS.consumers.length, lines: lines.size, decisions }).toEqual({
      calls: 9,
      lines: 8,
      decisions: 8, // one decision per LINE
    });
  });
  ...
  it("T6 — RECONCILIATION against the PRE-REPAIR tree: the consumer set is byte-for-byte the one WP93 inherited", () => {
    ...
    expect(afterCounts.get("files/file-ops.ts")).toBe(
      (beforeCounts.get("files/file-ops.ts") ?? 0) + 1,
    );
